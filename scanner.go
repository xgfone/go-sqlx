...
// panics or close application [sql.Scanner] implementations. A nil
// [rowbind.GeneralScanner.Value] discards the column.
//
...
// [time.Time] values retain their location unless [rowbind.GeneralScanner.Location] is
// explicitly set.
// Numeric timestamps are Unix seconds. Numeric durations use
// [rowbind.GeneralScanner.DurationUnit] (milliseconds by default), regardless
// of whether the source is integral.
...
// [RowScanner.Scan] borrows dst only for the call. Implementations must clone dst before
...
// RowCursor supplies [rowbind.Cursor.Next], [rowbind.Cursor.Err] and raw positional
// [rowbind.Cursor.Scan]. The scan method must write source column values into
// the supplied destinations, honoring [sql.Scanner], without
// applying another sqlx struct mapping or [ScanOptions] conversion layer. In
// particular, do not pass *[Rows] as a raw cursor: its [Rows.Scan] applies those policies
// even though its method set satisfies this interface. [Rows.Bind] passes *[sql.Rows].
//
// [rowbind.Cursor.Scan] borrows its argument slice; retaining it or a subslice requires a copy
